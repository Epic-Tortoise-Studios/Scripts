...
public class BoxDropTwoSides : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        PlayerBlock = GameObject.Find("***LEVEL_DEPENDENCIES***/BoxSpawns/PlayerBlock");
        //PlayerBlock.SetActive(false);
        PlayerBlocklocation = transform.position;

        PlayerBlocker = GameObject.Find("***LEVEL_DEPENDENCIES***/BoxSpawns/PlayerBlocker");
    ...
    void OnTriggerEnter(Collider BoxHoleTrigger)
    {
        ...
        if (BoxHoleTrigger.tag == "BoxDropper")
        {
            ...
            //Falling();
            Instantiate(PlayerBlock, new Vector3(transform.position.x, (transform.position.y - (transform.position.y /2)), transform.position.z), Quaternion.identity);
        }
        ...
        if (BoxHoleTrigger.tag == "BoxDropper2")
        {
            LandingSound.Play();
            Instantiate(PlayerBlock, new Vector3(transform.position.x, (transform.position.y - transform.position.y/2), transform.position.z),
                Quaternion.RotateTowards(transform.rotation, PlayerTransform.rotation, step));
            Falling.Play();
