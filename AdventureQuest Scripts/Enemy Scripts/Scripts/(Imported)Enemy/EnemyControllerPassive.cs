...
public class EnemyControllerPassive : MonoBehaviour
{
    ...
    public EnemyDamagePlayerSlow EnemyDamagePlayer;
    public EnemyHealth SlowEnemyHealth;
    public float healthamount;
    public float wickedamount = 75;

    public bool fleeing;
    public GameObject FleePoint;
    public Transform FleePointTransform;
    ...
    void Start()
    {
        ...
        Wicked = false;
        fleeing = false;
        FleePoint = GameObject.FindGameObjectWithTag("FleePoint");
        FleePointTransform = FleePoint.GetComponent<Transform>();
        ...
        EnemyDamagePlayer = GetComponentInChildren<EnemyDamagePlayerSlow>();
        EnemyDamageTimeout = EnemyDamagePlayer.damageTimeout;
        SlowEnemyHealth = GetComponent<EnemyHealth>();
        healthamount = SlowEnemyHealth.health;
        jumpAttacking = false;
    }
    ...
    void Update()
    {
        ...
        /*
        else if (healthamount <= wickedamount 
        && (Mathf.Abs(Vector3.Distance(enemy.transform.position, player.transform.position)) < chaseDistance) 
        && jumpAttacking == false && once < 4)
        {
            Invoke("Stop", 0f);
            Invoke("JumpAttack", 2f);
            Invoke("Stop", 6f);
            print("Wicked");
        }
        ...
        &&  ((Mathf.Abs(Vector3.Distance(enemy.transform.position, player.transform.position)) > chaseDistance)) 
            && InCombat == false && Patrolling == false && once < 4)
        {
            InCombat = false;
            Invoke("Patrol", 0f);
        ...
        && ((Mathf.Abs(Vector3.Distance(enemy.transform.position, player.transform.position)) > chaseDistance))
    && InCombat == false && Patrolling == true && once < 4  )
        {
            InCombat = false;
            Invoke("Patrol", 0f);
            //Invoke("ArrivedatPatrol", 1f);
        ...
        ////////////////Enemy Attacking Player Within Attack Range//////////////////////
        ...
        ///////////////////////Enemy Patrol, Player is outside Chase Distance/////////////////////////
        /*
        //Outside of ChaseDistance, Exited combat, Is Patrolling so started Stop and currently moving to Patrols
        ...
        else if (healthamount <= wickedamount && once < 4)
        {
            Invoke("Flee", 0f);
        ...
        else if (once == 5 && enemy.remainingDistance < 1)
        {
            
            enemyanim.SetBool("Dead", true);
            healthamount = SlowEnemyHealth.health = 0;
            Destroy(gameObject, 5);
            once = 7;
            print("Got to the Altar");
        }

        else if (((Mathf.Abs(Vector3.Distance(enemy.transform.position,
        player.transform.position)) > chaseDistance))
        && (!enemy.pathPending && enemy.remainingDistance < 2)
        && once < 4 && jumpAttacking == false)
        {
            print("Stopping at Patrol");
            Invoke("Stop", 0f);
            Invoke("StopPatrol", 0);
    ...
    {
        once = 5;
        enemy.destination = FleePoint.transform.position;
        print("fleeing");
        enemy.speed = 7;
    }
    void Pursue()
    ...
    void JumpAttack()
    {
        enemyanim.SetTrigger("JumpAttackTime");
        print("Doing Jump Attack");
        jumpAttacking = true;
    }
